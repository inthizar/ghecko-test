...
Process.loadData = function(data, fn) {
  ...
  if(insertData.length) {
    ...
    mongo.connect().then(function(db) {
      try {
        db.collection('transactions').insertMany(insertData, {writeConcern: {w:"majority"}, ordered: true}, function(err, result) {
          if(err) {
            console.error(err);
            return fn(null, {s: false, m: 'Import failed. please check the data is correct and try again'});
          }
